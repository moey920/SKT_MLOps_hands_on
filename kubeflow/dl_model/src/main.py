...
def katib_logger(name, namespace):
    ...
    return trial_details_log, optimal_trial_details_log


# config_op_packages = ["kubeflow-katib", "kfp"]

# katib_logger_op = components.func_to_container_op(katib_logger, packages_to_install=config_op_packages)
...
def build_model():
    for i in range(3):
        if optimizer_hp in str(config.optimizer_list[i]):
            _optimizer = config.optimizer_list[i]
    ...
    optimizer = _optimizer(lr=lr_hp, clipvalue=1.0)
